...
void PrintTimer() {
    ...
    std::cout << "Total time spent is: " << profilerMap["main"].accum
              << " seconds"  << std::endl;
    ...
    for (size_t i = 0; i < vec.size(); i++) {
        std::cout << "Percentage: " << std::setfill('0')
                  << std::setw(4) << vec[i].accum * 100.0
                  << "% Function name: " << vec[i].name
                  << ", calling times: " << vec[i].call_time
                  << std::endl;
        totalProfile += vec[i].accum;
    }
    std::cout << "Total profiled portion: " << totalProfile - 1
               << std::endl;
}
}